...
translate_budget <- function(hidden, metadata.row, action.row) {
  ...
  # Technical amendment 001: the sealed cohort generator calculated these
  # ratios correctly, but pmax() dropped their names before character lookup,
  # leaving the three CSV fields blank. Reconstruct exactly the predeclared
  # observable ratio from sealed year-4 spend and budget; no outcome truth is
  # used by this repair.
  observed.spend <- c(
...
for (business.id in selected.businesses) {
  ...
  cat(sprintf(
    "V10 AMSS truth worker %02d/%02d completed %s\n",
    worker.index + 1L,
    worker.count,
    business.id
  ))
