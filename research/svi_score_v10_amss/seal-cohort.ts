...
const PROTOCOL = resolve("research/svi_score_v10_amss/frozen-protocol/manifest.json");
...
if (observed.length !== 100 || hidden.length !== 100) {
  throw new Error(`V10 cohort is incomplete: ${observed.length} observed and ${hidden.length} hidden.`);
}
if (observed.some((name, index) => name.replace(/\.csv$/, "") !== hidden[index].replace(/\.rds$/, ""))) {
  throw new Error("Observed and sealed hidden V10 business identities do not match.");
}
...
if (metadata.rows.length !== 100 || experiments.rows.length !== 75) {
  throw new Error(`V10 expected 100 metadata rows and 75 experiments; found ${metadata.rows.length}/${experiments.rows.length}.`);
}
...
if ([...experimentCounts.values()].some((count) => count !== 1)) {
  throw new Error("A V10 business has more than one model-visible experiment.");
}
...
const manifest = {
  artifactId: "flux-v10-amss-sealed-cohort-manifest-v1",
  ...
  visibleWeeksPerBusiness: V10_AMSS_CONTRACT.time.visibleWeeks,
  experiments: experiments.rows.length,
  candidateFitsPlanned: V10_AMSS_CONTRACT.cohort.candidateFits,
  publicFiles,
  hiddenFiles,
