...
import { V10_AMSS_CHANNELS, V10_AMSS_CONTRACT, V10_AMSS_VERSION } from "./contract";

const RESEARCH_ROOT = resolve("research/svi_score_v10_amss");
const COHORT_ROOT = resolve(".flux-artifacts/svi-score-v10-amss/sealed-cohort");
const INFERENCE_ROOT = resolve(".flux-artifacts/svi-score-v10-amss/inference");
...
const REMOTE_PREFIX = "v10-amss-external-001";

export interface V10InferenceItem {
  businessId: string;
...
interface V10InferenceManifest {
  artifactId: string;
  version: string;
...
async function writeImmutable(path: string, bytes: Buffer): Promise<void> {
  try {
    ...
    if (digest(await readFile(path)) !== digest(bytes)) {
      throw new Error(`Immutable V10 SVI payload changed: ${path}.`);
    }
  }
}
...
async function loadProtocol() {
  const [manifestSource, candidateSource, scientificSource] = await Promise.all([
    ...
    readFile(resolve(RESEARCH_ROOT, "frozen-v9/scientific-contract.json")),
  ]);
  ...
  if (
    protocol.version !== V10_AMSS_VERSION ||
    ...
    digest(await readFile(resolve(RESEARCH_ROOT, "frozen-v9/selector.json"))) !==
      protocol.frozenV9SelectorSha256
  ) throw new Error("The V10 scientific protocol or frozen V9 selector is invalid.");
  ...
  if (candidates.length !== 24) throw new Error("V10 requires exactly 24 frozen specifications.");
  return {
...
async function loadCohort() {
  ...
  if (
    cohort.version !== V10_AMSS_VERSION ||
  ...
  ) throw new Error("The V10 sealed cohort is incomplete or its truth firewall is open.");
...
async function prepareBusiness(
  ...
  for (const evidenceArm of ["experiments-only", "benchmark-gap-fill"] as const) {
    const industryPriorChannels = evidenceArm === "benchmark-gap-fill"
      ? V10_AMSS_CHANNELS.filter((channel) => !experimentChannels.has(channel.toLowerCase()))
      : [];
...
async function worker(shard: number, shards: number): Promise<void> {
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    process.stdout.write(
      `V10 inference preparation ${shard + 1}/${shards}: ${index + 1}/${selected.length} businesses\n`,
    );
  }
...
async function runChild(shard: number, shards: number): Promise<void> {
  await new Promise<void>((done, reject) => {
    const child = spawn(process.execPath, [
      "--import",
      "tsx",
      resolve("research/svi_score_v10_amss/prepare-inference.ts"),
      "--worker",
      `--shard=${shard}`,
      `--shards=${shards}`,
    ...
    child.on("close", (code) => code === 0 ? done() : reject(
      new Error(`V10 inference preparation worker ${shard} exited ${code}.`),
    ));
  });
}
...
async function orchestrate(): Promise<void> {
  ...
  for (let shard = 0; shard < workers; shard += 1) {
    ...
    ) throw new Error(`V10 preparation part ${shard} is stale or invalid.`);
  ...
  if (
    items.length !== V10_AMSS_CONTRACT.cohort.candidateFits ||
    new Set(items.map((item) => item.inferenceFingerprint)).size !== items.length ||
    new Set(items.map((item) => item.businessId)).size !== 100
  ) throw new Error("The V10 inference manifest is incomplete or duplicated.");
  const manifest: V10InferenceManifest = {
    artifactId: "flux-v10-amss-inference-manifest-v1",
    version: V10_AMSS_VERSION,
    protocolFreezeSha256: protocol.protocolFreezeSha256,
    cohortManifestSha256: cohort.cohortManifestSha256,
  ...
  await atomicJson(CLOUD_MANIFEST_PATH, {
    ...
    sourceCheckpoint: "v10-amss-inference-manifest.json",
    failureCount: items.length,
    remotePrefix: REMOTE_PREFIX,
    protocolFreezeSha256: protocol.protocolFreezeSha256,
    auditVersion: V10_AMSS_VERSION,
