...
ROOT = Path(".flux-artifacts/svi-score-v10-amss")
INPUT_FILES = {
    ...
    "amendment_001": ROOT / "opened-truth/technical-amendment-001.json",
    "amendment_002": ROOT / "opened-truth/technical-amendment-002.json",
}
...
def verify_inputs() -> tuple[str, str]:
    amendment = json.loads(INPUT_FILES["amendment_002"].read_text(encoding="utf-8"))
    if amendment["status"] != "frozen-cloud-execution-only-amendment":
        raise RuntimeError("Cloud truth amendment is not frozen.")
    if amendment["cloudExecution"]["tasks"] != TASKS:
        raise RuntimeError("Cloud truth amendment does not declare 100 tasks.")

    expected = amendment["immutableInputs"]
    checks = {
        "candidateActionsSha256": INPUT_FILES["actions"],
        "truthOpenReceiptSha256": INPUT_FILES["receipt"],
        "cohortManifestSha256": INPUT_FILES["cohort_manifest"],
        "technicalAmendment001Sha256": INPUT_FILES["amendment_001"],
    }
    for field, path in checks.items():
        actual = sha256(path)
        if actual != expected[field]:
            raise RuntimeError(f"Immutable input hash mismatch for {field}: {actual}")
    return amendment["version"], amendment["amssSource"]["commit"]
...
def main() -> int:
    ...
    business_id = f"amss-v10-{task_index + 1:03d}"
