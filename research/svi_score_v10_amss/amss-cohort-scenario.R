# Parameterized AMSS scenario used only by the frozen V10 external audit.

v10_phase_index <- function() {
  c(
    rep(1L, 52L),
    rep(2L, 52L),
    rep(3L, 52L),
...
v10_phase_budgets <- function(base, growth) {
...
build_v10_amss_cohort_scenario <- function(design) {
...
v10_flux_channel_for_group <- function(evidence.group) {
  switch(
    ...
    stop("Unknown V10 evidence group: ", evidence.group)
  )
}

v10_raw_channel_for_flux <- function(flux.channel) {
  switch(
    as.character(flux.channel),
    "meta_acquisition_spend" = "paid_social_proxy",
    "google_search_nonbrand_spend" = "search",
    "ctv_spend" = "tv",
    stop("Unknown V10 Flux channel: ", flux.channel)
  )
}
