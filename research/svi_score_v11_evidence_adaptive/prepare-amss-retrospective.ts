...
import {
  V11_AMSS_RETROSPECTIVE_CONTRACT,
  V11_AMSS_RETROSPECTIVE_VERSION,
} from "./amss-retrospective-contract";

const AMSS_OBSERVABLE_PATH = resolve(
  ".flux-artifacts/svi-score-v10-amss/observable-freeze/observable-candidates.json",
);
...
if (
  ...
  observable.rows.length !== V11_AMSS_RETROSPECTIVE_CONTRACT.cohort.cachedSviFits
) {
...
const sets = [...grouped.entries()].sort().map(([businessId, rows]) => {
  if (
    rows.length !== V11_AMSS_RETROSPECTIVE_CONTRACT.cohort.candidatesPerBusiness ||
...
if (
  grouped.size !== V11_AMSS_RETROSPECTIVE_CONTRACT.cohort.businesses ||
...
const artifact = {
  artifactId: "flux-v11-amss-retrospective-observable-input-v1",
  version: V11_AMSS_RETROSPECTIVE_VERSION,
  generatedAt: new Date().toISOString(),
  status: "truth-blind-input-prepared-for-retrospective-selection",
  scientificLabel: V11_AMSS_RETROSPECTIVE_CONTRACT.scientificLabel,
  contract: V11_AMSS_RETROSPECTIVE_CONTRACT,
  provenance: {
    amssObservableSha256: sha256(observableSource),
