...
import type { Experiment, ModelConfig } from "../../lib/mmm/types";
...
function independentExperiments(business: SyntheticBusiness): Experiment[] {
  return business.truth.channels
    .filter((channel) => channel.channel !== "search")
    .map((channel) => ({
      channel: channel.spendColumn,
      startDate: String(business.observedRows[20].date),
      endDate: String(business.observedRows[35].date),
      incrementalOutcome: channel.targetRoi * 100_000,
      incrementalSpend: 100_000,
      standardError: Math.max(0.18, channel.targetRoi * 0.12),
      confidence: 0.9,
      scope: "total" as const,
      source: "Independent synthetic geo experiment",
    }));
}
...
async function evaluateScenario(
  ...
  const experiments = independentExperiments(business);
  ...
  for (const candidate of CANDIDATES) {
    ...
    const validationId = await validationFingerprint(
      ...
      experiments,
      {
        ...
        industryBenchmarkScreeningEnabled: false,
...
function markdownReport(report: ReturnType<typeof compactReport>): string {
  ...
  const candidateRows = [...report.candidates]
    ...
    .map(
      (candidate) =>
        `| ${candidate.id} | ${candidate.fluxScore?.toFixed(1) ?? "—"} | ${(candidate.weightedLogRoiError * 100).toFixed(1)}% | ${(candidate.weightedLogBenchmarkAgreement * 100).toFixed(1)}% | ${(candidate.contributionError * 100).toFixed(1)}% | ${(candidate.budgetRegret * 100).toFixed(1)}% |`,
    )
    .join("\n");
  return `# Score V2 pilot: ${report.scenario.label}\n\n${report.scenario.description}\n\nSeed: \`${report.scenario.seed}\`. Truth was held out of model fitting. The declared benchmark has ${(report.truth.weightedLogBenchmarkTruthError * 100).toFixed(1)}% spend-weighted log error versus causal truth and is recorded as fallible evidence, not an answer key.\n\n## Injected truth\n\n| Channel | Target ROI | Realized ROI |\n|---|---:|---:|\n${truthRows}\n\n## Candidate results\n\n| Candidate | Current Flux score | ROI truth error | Benchmark agreement error | Contribution error | Budget regret |\n|---|---:|---:|---:|---:|---:|\n${candidateRows}\n\nBudget regret is the share of the true incremental allocation opportunity lost by following the candidate instead of the hidden true optimum. Benchmark agreement is reported separately from causal truth and is never treated as the answer key. This single scenario is diagnostic evidence, not a learned score or a production claim.\n`;
}
...
function matrixMarkdown(summary: ReturnType<typeof matrixSummary>): string {
  ...
  return `# Score V2 pilot matrix\n\nThis report contains ${summary.candidateCount} candidate evaluations across ${summary.scenarioCount} deterministic synthetic businesses.\n\n- Correlation of current Flux score with lower budget regret: **${scoreRegret === null ? "not estimable" : scoreRegret.toFixed(3)}**\n- Correlation of current Flux score with lower ROI error: **${scoreRoi === null ? "not estimable" : scoreRoi.toFixed(3)}**\n- Mean regret of the current Flux winner: **${(summary.selection.meanCurrentWinnerRegret * 100).toFixed(1)}%**\n- Mean regret of the best evaluated candidate: **${(summary.selection.meanLowestAvailableRegret * 100).toFixed(1)}%**\n- Current score selected a lowest-regret candidate in **${(summary.selection.lowestRegretSelectionRate * 100).toFixed(1)}%** of scenarios.\n\n| Scenario | Current Flux winner | Winner regret | Lowest-regret candidate | Lowest regret |\n|---|---|---:|---|---:|\n${rows}\n\nThis is a feasibility diagnostic over a deliberately small candidate set. It is not sufficient to estimate new production weights.\n`;
}
