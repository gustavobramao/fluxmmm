...
def add_title(story: list[Flowable]) -> None:
    story.extend([
        ...
        P(
            "Marketing mix modeling (MMM) is commonly selected by predictive fit, residual diagnostics, calibration agreement, or fixed analyst scores, although its operational purpose is budget decision support. RegretSet-MMM instead learns across advertisers which complete Bayesian MMM specification is least likely to produce costly budget decisions. Forty-eight truth-blind candidate MMMs per advertiser are fitted with PyMC FullRankADVI, a full-rank automatic-differentiation method in the stochastic variational inference family, and represented by 232 deployment-observable tokens covering diagnostics, posterior decision geometry, temporal identification, evidence attribution, and assumptions. Evidence-adaptive RegretSet-MMM retains the original joint all-token path and adds a business-level gate that routes residual representations of predictive, causal-identification, posterior-decision, and structural-specification evidence according to the evidence available for each advertiser. A permutation-invariant multi-head learner predicts loss across four predeclared budget decisions. We establish a finite-candidate bound connecting selected capped regret to regret-weighted oracle misrankings and decompose local ROI information among observational data, experiments, benchmarks, and regularization. The frozen evidence-adaptive selector was evaluated without retraining in a fresh cohort of 100 businesses generated by Google's independently developed Aggregate Marketing System Simulator. Across 4,800 candidate fits and 19,200 frozen actions, its capped decision-risk objective was 0.203, versus 0.353 for prediction-only selection, 0.401 for RegretSet-MMM, 0.458 for uniform random valid selection, 0.701 for classic Pareto selection, and 0.029 for an unattainable in-pool oracle. The two predeclared comparator tests and the P95 tail-safety check passed. These findings establish confirmatory external synthetic transport under the declared candidate and utility contracts, not real-advertiser effectiveness or universal state-of-the-art performance.",
            "abstract",
        ),
...
def add_external_validation(story: list[Flowable]) -> None:
    ...
    story.extend([
        ...
        P(
            f"Evidence-adaptive RegretSet-MMM achieved a decision-risk objective of {regretset['selectionObjective']:.3f}. RegretSet-MMM achieved {frozen_v9['selectionObjective']:.3f}; the paired evidence-adaptive-minus-RegretSet-MMM difference was {v9_paired['selectionObjectiveDifference']:.3f}, with 95% bootstrap interval [{v9_paired['selectionObjectiveDifferenceInterval95'][0]:.3f}, {v9_paired['selectionObjectiveDifferenceInterval95'][1]:.3f}]. Prediction-only selection achieved {prediction_only['selectionObjective']:.3f}; the paired difference was {prediction_paired['selectionObjectiveDifference']:.3f}, with interval [{prediction_paired['selectionObjectiveDifferenceInterval95'][0]:.3f}, {prediction_paired['selectionObjectiveDifferenceInterval95'][1]:.3f}]. Thus the evidence-adaptive selector reduced the objective by {objective_reductions['v9']:.1f}% relative to RegretSet-MMM and {objective_reductions['prediction']:.1f}% relative to prediction-only selection."
        ),
        P(
