...
def save_amss_external_validation() -> Path:
    ...
    for values, color, label, width in [
        (regretset, "#355C9A", "Evidence-adaptive RegretSet-MMM", 2.2),
        (prediction, "#C28A42", "Prediction-only", 1.4),
        (frozen_v9, "#777777", "RegretSet-MMM", 1.4),
    ...
    ax = axes[1]
    labels = ["Oracle", "Evidence-adaptive\nRegretSet-MMM", "Prediction\nonly", "RegretSet-\nMMM", "Random\nvalid", "Classic\nPareto"]
    objectives = [
...
def add_title(story: list[Flowable]) -> None:
    story.extend([
        Spacer(1, 3 * mm),
        P(TITLE, "title"),
        P("Evidence-adaptive set-wise learning, economic-regret supervision, and source-resolved posterior attribution", "subtitle"),
        P("Gustavo Bramao", "author"),
        P("Independent Researcher | September 2026 | Working paper - external transport audit complete", "date"),
        HRFlowable(width="100%", thickness=0.5, color=BLACK, spaceBefore=1, spaceAfter=8),
        P("Abstract", "abstract_heading"),
        P(
            "Marketing mix model specifications are commonly selected using predictive fit, residual diagnostics, calibration agreement, or fixed analyst scores, even though their operational purpose is to support budget decisions. RegretSet-MMM instead learns across advertisers which complete Bayesian MMM specification is least likely to produce costly budget decisions. Forty-eight truth-blind candidate MMMs per advertiser are fitted with PyMC FullRankADVI, a full-rank automatic-differentiation method in the stochastic variational inference family, and represented by 232 deployment-observable tokens covering diagnostics, posterior decision geometry, temporal identification, evidence attribution, and assumptions. Evidence-adaptive RegretSet-MMM retains the shared 232-token candidate-set pathway and adds a business-level gate that routes residual representations of predictive, causal-identification, posterior-decision, and structural-specification evidence according to the evidence available for each advertiser. A permutation-invariant multi-head learner predicts loss across four prespecified budget decisions. We establish a finite-candidate bound connecting selected capped regret to regret-weighted oracle misrankings and decompose local ROI information among observational data, experiments, benchmarks, and regularization. After development was completed, the evidence-adaptive selector was evaluated without retraining in a fresh cohort of 100 businesses generated by Google's independently developed Aggregate Marketing System Simulator. Across 4,800 candidate fits and 19,200 budget actions fixed before synthetic truth was revealed, its capped decision-risk objective was 0.203, versus 0.353 for prediction-only selection, 0.401 for RegretSet-MMM, 0.458 for uniform random valid selection, 0.701 for classic Pareto selection, and 0.029 for an unattainable in-pool oracle. The prespecified comparisons against RegretSet-MMM and prediction-only selection passed, together with the P95 tail-safety criterion. These findings support transport across synthetic generators under the stated candidate and utility contracts; they do not establish real-advertiser effectiveness or universal state-of-the-art performance.",
            "abstract",
        ),
...
def add_introduction(story: list[Flowable]) -> None:
    story.extend([
        ...
        caption("Figure 1", "Research and deployment paths. Hidden causal truth is opened only after a candidate has acted in simulation. Deployment uses the fixed mapping from observable candidate sets to predicted decision risk."),
        ...
        bullet("A 232-token, truth-blind representation combining classical diagnostics with FullRankADVI posterior geometry, channel-response uncertainty, cross-channel posterior dependence, evidence provenance, and model assumptions."),
        ...
        bullet("An auditable validation design using advertiser-grouped nested cross-validation, complete-mechanism holdouts, a one-time internal sealed cohort, and a fresh external transport audit in Google's independently developed Aggregate Marketing System Simulator."),
        Spacer(1, 2 * mm),
        H2("1.2", "Claim boundary"),
        P(
            "The contribution is a method for learning and auditing an MMM selection rule. An internal sealed audit evaluates RegretSet-MMM on 140 previously unseen advertisers generated by the study's synthetic data-generating process. A separate external audit evaluates evidence-adaptive RegretSet-MMM in the independently developed AMSS simulator against RegretSet-MMM, prediction-only selection, classic Pareto selection, and uniform random valid selection, with an in-pool oracle reported only as an unattainable lower reference. These synthetic results do not imply real-advertiser effectiveness, universal state-of-the-art performance, inclusion of the true model in the candidate library, or exact Bayesian posterior inference."
        ),
    ])
...
def add_related_work(story: list[Flowable]) -> None:
    story.extend([
        ...
        P(
            "DeepSets provides a general architecture for permutation-invariant functions over unordered collections [14]. Here candidate order is arbitrary, so a shared encoder and symmetric pooling operator supply advertiser-specific context without creating a candidate-ID signal. Candidate posteriors are fitted with PyMC FullRankADVI, a full-rank automatic-differentiation variational inference procedure within the broader stochastic variational inference family. It scales to tens of thousands of fits but may understate tails or miss multimodality [15,16]. The selection framework is inference-engine agnostic in principle: the same posterior tokens can be computed from more accurate draws when resources permit."
        ),
        H2("2.5", "Positioning of the contribution"),
        P(
            "The scientific question is whether deployment-observable evidence predicts the economic consequences of selecting among plausible MMMs for unseen advertisers. The internal audit compares RegretSet-MMM with a frozen restricted-diagnostic baseline that uses the same candidate set and economic objective but only a smaller predeclared subset of observable diagnostics. This comparison tests whether the complete token representation adds economic-ranking information; the restricted baseline is not proposed as a production selector."
        ),
        P(
...
def add_candidate_models(story: list[Flowable]) -> None:
    story.extend([
        ...
        P(
            "All research candidates use PyMC 6.2 FullRankADVI under a fixed inference contract. A multivariate Gaussian variational family q_φ(θ) is optimized by maximizing the evidence lower bound (ELBO), retaining covariance that a mean-field approximation would discard. Posterior draws from q_φ feed response, uncertainty, and decision tokens. Multiple convergence and reproducibility checks guard against unstable variational solutions."
        ),
        equation(r"\phi^*=\arg\max_{\phi}\left\{\mathbb{E}_{q_{\phi}(\theta)}[\log p(D,E,\theta)]-\mathbb{E}_{q_{\phi}(\theta)}[\log q_{\phi}(\theta)]\right\}", "10"),
        P(
            "FullRankADVI is used because tens of thousands of posterior fits are required. It is not assumed superior to Markov chain Monte Carlo. Like other variational methods, FullRankADVI can smooth multimodal posteriors and distort tail mass, so the inference engine is part of the reported research contract. The learned selector is compatible with posterior draws from NUTS in principle, provided the 232-token contract is computed identically."
        ),
...
def add_tokens(story: list[Flowable]) -> None:
    story.extend([
        ...
        P(
            "The representation contains 136 base tokens and 96 posterior decision tokens, for 232 per candidate. Base tokens cover predictive generalization, residual structure, causal and placebo stress, ROI coherence, temporal evidence, evidence quality and conflict, model assumptions, and their allowed interactions. Posterior tokens summarize channel-level ROI geometry, response uncertainty, evidence provenance, contribution stability, and cross-channel dependence from FullRankADVI draws."
        ),
        make_table([
        ...
        make_table([
            ...
            ["Decision safety", "implausible probability, near-zero probability, skewness, analytic-to-FullRankADVI location shift, interval overlap"],
        ...
        caption("Table 4", "Ninety-six FullRankADVI posterior decision tokens. All are computed without synthetic truth."),
...
def add_learning(story: list[Flowable]) -> None:
    story.extend([
        ...
        caption("Figure 3", "Core set-wise, multi-head pathway used by RegretSet-MMM and retained as the joint all-token path in evidence-adaptive RegretSet-MMM. Reordering candidates or channels cannot change candidate predictions. Equation (16a) adds the evidence-gated residual pathway without removing this joint representation."),
        ...
        H2("8.4", "From RegretSet-MMM to evidence-adaptive RegretSet-MMM"),
        P(
            "RegretSet-MMM uses one pooled 232-token DeepSets pathway for every advertiser. It can learn interactions involving experiment and benchmark indicators, but it has no separate representation of the advertiser's overall evidence environment. Evidence-adaptive RegretSet-MMM preserves that joint all-token pathway and adds four disjoint residual encoders for predictive generalization, causal identification, posterior decision quality, and structural specification. A 63-token, candidate-independent context records evidence availability and channel coverage, explicit missingness masks, candidate-set regime, set-level diagnostic summaries, and matched benchmark-minus-experiment summaries."
        ),
        multiline_equation([
        ...
        P(
            "Thus, the two selectors share the same fitted candidate posteriors, 232 candidate tokens, economic-loss targets, and set-wise learning principle. Their sole architectural distinction is that evidence-adaptive RegretSet-MMM also observes 63 advertiser-level context tokens and uses them to vary the residual contribution of four evidence-specific encoders."
        ),
        P(
...
def add_evidence_theory(story: list[Flowable]) -> None:
    story.extend([
        ...
        caption("Figure 4", "Estimated local evidence attribution for the 1,260 channel posteriors in the 420 outer-fold selected development candidates. Bars report equal-channel mean Laplace curvature shares. Overall, observational data supplied 42.1%, experiments 36.9%, benchmarks 7.0%, and regularization 14.0% of local ROI information. These are development-cohort influence estimates, not scores of source validity."),
        P(
...
def add_validation(story: list[Flowable]) -> None:
    story.extend([
        ...
        caption("Table 5", "Predeclared confirmatory endpoints and their roles."),
...
def add_confirmatory_results(story: list[Flowable]) -> None:
    ...
    story.extend([
        H1("11", "Internal sealed audit of RegretSet-MMM"),
        P(
            "The one-time internal audit evaluated RegretSet-MMM on 140 previously unseen advertisers generated by the study's synthetic data-generating process and 6,720 newly fitted candidate MMMs. It provides an independent test of the foundational set-wise method; the evidence-adaptive extension is evaluated separately in the AMSS transport study in Section 12. All 6,720 candidate rows were finite, valid, and unique. The audit was scored once after posterior inference and truth-blind token construction were complete. All nine predeclared confirmatory checks passed."
        ),
        make_table([
        ...
        P(
            "The primary superiority check, full-coverage mean and P90 improvements, P95 non-inferiority, dangerous-false-champion improvement, selective-risk-area improvement, matched 70% objective improvement, theorem verification, and FullRankADVI-only inference check all passed. The finite-candidate bound had zero violations. The result therefore confirms synthetic generalization under the frozen cohort contract. It does not establish real-world generalization, global optimality, or realized advertiser profit improvement. Any future selector tuning requires a new sealed cohort."
        ),
        H2("11.4", "Audit integrity"),
        P(
            "All 6,720 posterior fits and labels satisfied the fixed scientific contract. An orchestration indexing error affected which jobs were launched, not the models or analysis. The 864 missing candidate labels were recovered under an indexing-only correction before the audit was scored. No posterior artifact, selector, token, endpoint, or hidden outcome was changed or inspected during recovery. Appendix D provides the complete machine-readable provenance receipt."
        ),
    ])
...
def add_external_validation(story: list[Flowable]) -> None:
    ...
    story.extend([
        ...
        P(
            "The Aggregate Marketing System Simulator (AMSS) was developed independently of FluxMMM to generate aggregate marketing time series and associated ground-truth return and marginal-return measures [23]. It therefore provides a stronger transport test than drawing additional seeds from the study's own generator. A new cohort, with seeds disjoint from the earlier AMSS study, was generated only after the evidence-adaptive architecture, 232 candidate tokens, 63 business-context tokens, FullRankADVI contract, candidate library, evidence rules, four decisions, loss definition, comparator family, and 65/35 mean-tail utility were frozen. No AMSS truth was available during fitting or selection, and no model was retrained or reselected after truth opening."
        ),
        P(
        ...
        P(
            "The design uses a deterministic 100-point marginal space-filling cohort and rotates all six orders of the three media modules. Candidate fitting observes 156 weekly periods after a 52-week burn-in. Budget actions fixed before truth disclosure cover a 52-week decision horizon plus eight post-action carryover weeks. Every candidate chooses the feasible allocation that maximizes posterior-expected incremental contribution less incremental media cost. AMSS then evaluates the fixed action using four common-random-number truth replicates. Period-matched experiments, when assigned, compare treatment with a 20%-lower-budget path from the same pretest state and include a declared post-test outcome window."
        ),
        H2("12.2", "Selection rules and benchmark hierarchy"),
        definition("Evidence-adaptive RegretSet-MMM", "The focal learned rule. It retains RegretSet-MMM's joint candidate-set representation and adds the advertiser-level evidence context and residual gate described in Section 8.4. It sees only truth-blind deployment tokens."),
        definition("RegretSet-MMM", "The frozen non-adaptive learned rule. It uses the same 232 candidate tokens, economic-loss targets, and joint candidate-set pathway but no 63-token advertiser evidence context or gated residual experts."),
        ...
        definition("Classic Pareto", "Forms a nondominated set over rolling prediction error, DECOMP.RSSD, and same-window experiment MAPE when available, then applies a deterministic equal-rank compromise. It is reported as a secondary descriptive benchmark."),
        ...
        make_table([
            ...
            ["Evidence-adaptive RegretSet-MMM", "confirmatory target", f"{regretset['mean']:.3f}", f"{regretset['p90']:.3f}", f"{regretset['p95']:.3f}", f"{regretset['selectionObjective']:.3f}"],
            ...
            ["RegretSet-MMM", "primary comparator", f"{frozen_v9['mean']:.3f}", f"{frozen_v9['p90']:.3f}", f"{frozen_v9['p95']:.3f}", f"{frozen_v9['selectionObjective']:.3f}"],
            ...
            ["Classic Pareto", "secondary benchmark", f"{conventional['mean']:.3f}", f"{conventional['p90']:.3f}", f"{conventional['p95']:.3f}", f"{conventional['selectionObjective']:.3f}"],
        ...
        caption("Table 8", "Fresh AMSS capped decision risk. The objective is 65% cohort mean plus 35% cohort P90; lower is better. Uniform-random rows summarize each business's conditional expected risk. Classic Pareto is a secondary descriptive benchmark. The oracle is not deployable."),
        H2("12.3", "Confirmatory result"),
        P(
            f"Evidence-adaptive RegretSet-MMM achieved a decision-risk objective of {regretset['selectionObjective']:.3f}. RegretSet-MMM achieved {frozen_v9['selectionObjective']:.3f}; the paired difference (evidence-adaptive RegretSet-MMM minus RegretSet-MMM) was {v9_paired['selectionObjectiveDifference']:.3f}, with 95% bootstrap interval [{v9_paired['selectionObjectiveDifferenceInterval95'][0]:.3f}, {v9_paired['selectionObjectiveDifferenceInterval95'][1]:.3f}]. Prediction-only selection achieved {prediction_only['selectionObjective']:.3f}; the paired difference was {prediction_paired['selectionObjectiveDifference']:.3f}, with interval [{prediction_paired['selectionObjectiveDifferenceInterval95'][0]:.3f}, {prediction_paired['selectionObjectiveDifferenceInterval95'][1]:.3f}]. Thus evidence-adaptive RegretSet-MMM reduced the objective by {objective_reductions['v9']:.1f}% relative to RegretSet-MMM and {objective_reductions['prediction']:.1f}% relative to prediction-only selection."
        ),
        P(
            f"All three predeclared checks passed: the evidence-adaptive objective was lower than RegretSet-MMM, no higher than prediction-only selection, and its P95 capped risk of {regretset['p95']:.3f} remained well inside the 0.02 non-inferiority margin relative to the better comparator tail. Both paired objective intervals exclude zero in the favorable direction. The bootstrap resampled businesses within the four evidence groups for 10,000 replicates, preserving the design's evidence balance."
        ),
        image_flow(save_amss_external_validation(), 164),
        caption("Figure 7", "Fresh AMSS transport. Panel A shows business-level capped-risk distributions for deployable selectors and the in-pool oracle. Panel B reports the common 65/35 mean-tail objective. Classic Pareto is a secondary descriptive benchmark."),
        H2("12.4", "Secondary classic Pareto benchmark"),
        P(
            "Classic Pareto approximates established multi-objective MMM practice [7]. It forms the nondominated set over rolling prediction error, DECOMP.RSSD between analytic-screening spend and contribution shares, and same-window experiment MAPE when an experiment exists. A deterministic equal-rank compromise selects one member; no-experiment businesses use prediction and decomposition only."
        ),
        P(
            f"Classic Pareto's objective was {conventional['selectionObjective']:.3f}. Evidence-adaptive RegretSet-MMM was {objective_reductions['pareto']:.1f}% lower. Its business-level mean-risk difference was {conventional_paired['meanDifference']:.3f}, with 95% interval [{conventional_paired['interval95'][0]:.3f}, {conventional_paired['interval95'][1]:.3f}]. Evidence-adaptive RegretSet-MMM had lower risk for {conventional_paired['selectedMethodLowerRisk']} businesses, tied for {conventional_paired['tied']}, and higher risk for {conventional_paired['comparatorLowerRisk']}. The observed comparison favors evidence-adaptive RegretSet-MMM over this specified Pareto rule in this cohort; it is not a claim about every Pareto implementation, expert review process, or established MMM system."
        ),
        H2("12.5", "Interpretation and remaining headroom"),
        P(
            f"The descriptive uniform-random objective was {random_valid['selectionObjective']:.3f}, {objective_reductions['random']:.1f}% above evidence-adaptive RegretSet-MMM. The evidence-group-stratified paired mean-risk difference against each business's random-valid expectation was {paired['point']:.3f}, with 95% interval [{paired['interval'][0]:.3f}, {paired['interval'][1]:.3f}]. The in-pool oracle achieved an objective of {oracle['selectionObjective']:.3f}. The gap from {regretset['selectionObjective']:.3f} demonstrates substantial headroom even without expanding the candidate library. The result supports external transport of the learned ranking rule, not global optimality."
        ),
        P(
        ...
        P(
            f"All {truth_receipt['parts']} truth partitions and {truth_receipt['uniqueCandidateScenarioRows']:,} unique candidate-scenario outcomes were retrieved and hash-verified. The protocol forbade post-truth retraining, reselection, and business exclusion. These controls support the prespecified transport comparisons against RegretSet-MMM and prediction-only selection; they do not establish realized effects in advertiser data or universal superiority to every published MMM workflow."
        ),
    ])
...
def add_interpretation(story: list[Flowable]) -> None:
    story.extend([
        ...
        caption("Figure 8", "RegretSet-MMM advertiser-grouped development importance. Bars show the percentage change in held-out economic loss when a complete token group is removed and RegretSet-MMM is relearned. Positive values indicate incremental predictive information; gray negative values indicate conditional redundancy or finite-sample overfit, not that the underlying scientific check is unnecessary. The sealed cohort was not opened for this analysis."),
        P(
        ...
        make_table([
            ...
            ["Posterior decision geometry", "ROI levels and intervals; tail mass; contribution stability; response uncertainty; channel covariance; FullRankADVI reliability", "+38.7%. Inspect the entire posterior allocation geometry before promoting a model."],
        ...
        caption("Table 10", "Practitioner interpretation of the RegretSet-MMM outer-fold development ablation. Percentages are conditional predictive importance within the study's synthetic development data, not evidence-adaptive routing weights, universal validation weights, or confirmatory audit outcomes."),
        H2("13.3", "Interpreting non-positive ablation estimates"),
        P(
...
def add_limitations(story: list[Flowable]) -> None:
    story.extend([
        ...
        bullet("Approximate posterior inference: FullRankADVI may distort skew, multimodality, and tail dependence. All reported candidate posteriors use the same FullRankADVI contract, so the conclusions are conditional on that approximation."),
        ...
        P(
            "The completed internal and AMSS audits establish transfer only under their stated synthetic populations, candidate pool, evidence rules, and decision utility. They do not establish universal state-of-the-art MMM performance, global optimality, superiority to expert modelers, or real-world transport across industries. Nor do they imply that evidence influence proves evidence quality, residual diagnostics prove exogeneity, or the finite-candidate regret bound proves population generalization."
        ),
    ])
...
def add_open_science(story: list[Flowable]) -> None:
    story.extend([
        ...
        make_table([
            ...
            ["Candidate fitting", "observed data and declared evidence", "candidate and FullRankADVI fingerprint"],
            ...
            ["External AMSS audit", "budget actions fixed before truth disclosure; AMSS truth opened once", "adapter contract, actions, hashes, conditional random benchmark, verified truth receipt"],
...
def add_conclusion(story: list[Flowable]) -> None:
    story.extend([
        ...
        P(
            "The internal sealed audit supports RegretSet-MMM on previously unseen advertisers generated by the study's synthetic data-generating process. More importantly, a fresh AMSS cohort showed that evidence-adaptive RegretSet-MMM lowered the 65/35 mean-tail decision-risk objective relative to RegretSet-MMM and prediction-only selection, with favorable paired bootstrap intervals and preserved P95 tail safety. Classic Pareto and uniform random valid selection produced higher loss under their stated rules. Because no fresh-cohort truth was available during selector training, candidate fitting, or action selection, the result demonstrates that deployment-observable posterior and validation evidence can carry useful economic-ranking information across synthetic data-generating systems."
        ),
        P(
            "The sizeable gap to the in-pool oracle, the restricted channel archetypes, the FullRankADVI approximation, and the absence of real-advertiser causal outcomes define the next research frontier. RegretSet-MMM should therefore be read as an externally transportable and falsifiable decision-focused selection protocol, not a claim of universal model recovery. Its open implementation allows researchers and advertisers to reproduce the evidence, test new simulators and candidate libraries, and challenge the learned rule under new decision utilities."
        ),
    ])
...
def add_appendices(story: list[Flowable]) -> None:
    ...
    story.extend([
        ...
        make_table([
            ...
            ["External precommitted actions", "19,200 across four budget decisions"],
        ], widths=[67 * mm, 95 * mm]),
        ...
        make_table([
            ...
            ["Protocol", "fresh AMSS confirmatory audit; frozen before truth"],
            ...
            ["Frozen selector", "Evidence-adaptive RegretSet-MMM; 232 candidate + 63 context tokens"],
            ...
            ["Evaluated selection rules", "Evidence-adaptive RegretSet-MMM; RegretSet-MMM; prediction-only; uniform random valid; classic Pareto; in-pool oracle reference"],
            ["Truth verification", f"{amss_truth_receipt['parts']} partitions; {amss_truth_receipt['uniqueCandidateScenarioRows']:,} unique outcomes"],
            ["Confirmatory checks", "Evidence-adaptive RegretSet-MMM vs RegretSet-MMM; Evidence-adaptive RegretSet-MMM vs prediction-only; P95 tail safety - all passed"],
            ["Evidence-adaptive development SHA-256", v11_development_hash],
        ...
        P(
            f"The descriptive paired interval resamples businesses within each of the four 25-business evidence groups. With {amss_paired['resamples']:,} resamples and seed {amss_paired['seed']}, evidence-adaptive RegretSet-MMM minus expected-random mean risk was {amss_paired['point']:.3f}, with 95% interval [{amss_paired['interval'][0]:.3f}, {amss_paired['interval'][1]:.3f}]. Random-valid was not one of the two primary confirmatory comparators."
        ),
        ...
        H2("E.4", "Secondary classic Pareto receipt"),
        make_table([
            ["Secondary item", "Verified value"],
            ["Status", "secondary descriptive benchmark; outside the two prespecified confirmatory comparisons"],
        ...
        caption("Table E.2", "Reproducibility and governance receipt for the secondary classic Pareto benchmark."),
    ])
