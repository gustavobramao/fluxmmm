...
def add_limitations(story: list[Flowable]) -> None:
    story.extend([
        ...
        P("Advertiser-grouped cross-fitting prevents a candidate or matched regime from leaking into the training fold of its own assessment advertiser. It does not erase researcher adaptation. Architectures, targets, and representations were developed using summaries from the same 420-advertiser corpus. Reported differences are therefore model-development evidence and may be optimistic [20]. A future study should freeze this specification and evaluate it once on independently generated populations before making confirmatory claims."),
        ...
        P("Code, frozen token registries, simulator contracts, candidate fingerprints, cross-fitted comparison receipts, and artifact hashes are available in the public repository and OSF reproducibility archive. The final comparison and importance analyses reuse 20,160 stored posterior fits. Reproduction should verify hashes before rebuilding tables and figures."),
        bullet("Repository: https://github.com/gustavobramao/fluxmmm"),
        bullet("Reproducibility archive: https://osf.io/becgp/"),
        H2("12.6", "Priority extensions"),
        bullet("Independent synthetic populations with different equations, channel systems, and evidence mechanisms."),
        bullet("Prospective advertiser studies where experiments or later outcomes can partially validate model ranking."),
...
def add_conclusion(story: list[Flowable]) -> None:
    story.extend([
        ...
        P("The result is informative but conditional. It shows that posterior and validation evidence can predict economic differences among fitted models in a heterogeneous synthetic population; it does not prove real-advertiser causal validity. The most defensible next step is not a stronger slogan but a frozen, independent test of the present specification. The paper supplies the method, information-time contract, comparative evidence, and reproducibility artifacts needed for that test."),
    ])
