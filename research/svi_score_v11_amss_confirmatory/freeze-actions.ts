...
import { recommendV11ConfirmatoryCandidateActions } from "./actions";
import { V11_AMSS_CONFIRMATORY_CHANNELS, V11_AMSS_CONFIRMATORY_VERSION } from "./contract";
...
async function writeImmutable(path: string, value: string): Promise<void> {
  try {
    ...
    if ((await readFile(path, "utf8")) !== value) {
      throw new Error(`Frozen V11 confirmatory observable artifact changed: ${path}.`);
    }
  }
}
...
if (
  inference.version !== V11_AMSS_CONFIRMATORY_VERSION ||
  inference.hiddenTruthDereferenced ||
  inference.items.length !== 4_800
) throw new Error("V11 confirmatory inference manifest is incomplete or opened hidden truth.");

const frozenV11 = JSON.parse(
  await readFile(resolve("research/svi_score_v11_amss_confirmatory/frozen-selectors/v11-selector.json"), "utf8"),
) as { featureNames: string[] };
const tokenRegistry = { count: frozenV11.featureNames.length, names: frozenV11.featureNames };
...
for (const item of inference.items) {
  ...
  ) throw new Error(`Invalid V11 confirmatory SVI result: ${item.inferenceFingerprint}.`);
...
for (const business of inference.businesses) {
  ...
  spendByBusiness.set(business.businessId, Object.fromEntries(
    V11_AMSS_CONFIRMATORY_CHANNELS.map((channel) => [
      channel,
...
const rows = inference.items.map((item) => {
  ...
  ) throw new Error(`V11 confirmatory token contract changed for ${item.businessId}/${item.candidateId}.`);
  const business = businessById.get(item.businessId)!;
  const actions = recommendV11ConfirmatoryCandidateActions({
...
) throw new Error("V11 confirmatory observable candidate/action freeze is incomplete.");

const dataset = {
  artifactId: "flux-v11-amss-confirmatory-observable-candidate-set-v1",
...
const manifest = {
  artifactId: "flux-v11-amss-confirmatory-observable-freeze-manifest-v1",
  version: V11_AMSS_CONFIRMATORY_VERSION,
