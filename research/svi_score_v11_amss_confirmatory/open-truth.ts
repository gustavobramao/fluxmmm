...
async function runWorker(index: number, workers: number): Promise<void> {
  await new Promise<void>((done, reject) => {
    const child = spawn("Rscript", [
      resolve("research/svi_score_v11_amss_confirmatory/open-truth.R"),
      `--worker=${index}`,
      `--workers=${workers}`,
    ], { cwd: process.cwd(), env: process.env, stdio: "inherit" });
    child.on("error", reject);
    child.on("close", (code) => code === 0 ? done() : reject(
      new Error(`V11 confirmatory AMSS truth worker ${index} exited ${code}.`),
    ));
  });
}
...
const selections = JSON.parse(selectionsSource.toString("utf8")) as {
  ...
  provenance: {
    ...
    hiddenEconomicOutcomesReadBySelection: boolean;
  };
};
if (
  cohort.version !== V11_AMSS_CONFIRMATORY_VERSION || observable.version !== V11_AMSS_CONFIRMATORY_VERSION ||
  selections.version !== V11_AMSS_CONFIRMATORY_VERSION || cohort.businesses !== 100 ||
  ...
  cohort.truthOpened || observable.truthOpened ||
  selections.provenance.hiddenEconomicOutcomesReadBySelection ||
  selections.status !== "all-comparator-selections-frozen-before-amss-truth-open"
) throw new Error("The V11 confirmatory truth firewall cannot open: a frozen input is incomplete or inconsistent.");

const receiptOnly = process.argv.includes("--receipt-only");
const workers = receiptOnly
  ? 100
  : Math.max(1, Math.min(argument("workers", Math.min(8, cpus().length)), 16));
const receipt = {
  artifactId: "flux-v11-amss-confirmatory-truth-open-receipt-v1",
