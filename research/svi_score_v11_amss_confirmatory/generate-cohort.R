...
library(data.table)
source(file.path("research", "svi_score_v11_amss_confirmatory", "amss-cohort-scenario.R"))
...
if (dir.exists(sealed.root) || dir.exists(cohort.root)) {
  stop("The V11 confirmatory AMSS cohort or its staging directory already exists; immutable regeneration is forbidden.")
}
...
design.path <- file.path(
  "research",
  "svi_score_v11_amss_confirmatory",
  "frozen-protocol",
  "cohort-design.csv"
)
...
for (business.index in seq_len(nrow(design))) {
  ...
  scenario <- build_v11c_amss_cohort_scenario(row)
  ...
  utilization <- setNames(
    pmax(0.05, pmin(2, year4.spend / year4.budget[names(year4.spend)])),
    names(year4.spend)
  )
  stopifnot(all(is.finite(utilization)))

  experiment.channel <- v11c_flux_channel_for_group(row$evidence_group)
  if (!is.na(experiment.channel)) {
    raw.channel <- v11c_raw_channel_for_flux(experiment.channel)
  ...
  cat(sprintf("V11 confirmatory AMSS cohort %03d/100 %s\n", business.index, row$business_id))
}
...
writeLines(
  c(
    "artifact_id=flux-v11-amss-confirmatory-sealed-cohort-v1",
    "businesses=100",
